...
from app.schemas.student import StudentCourseItem, TranscriptParseResult
  ...
  "courses": [
    {
      ...
      "grade": "<grade as string, e.g. '1,3' or 'bestanden' or null>",
...
def _compute_gpa(courses: list[StudentCourseItem]) -> float | None:
    ...
    for c in courses:
        if c.grade is None or c.credits is None:
            continue
        # Normalise German comma decimal to dot.
        grade_str = c.grade.replace(",", ".")
        try:
            grade_val = float(grade_str)
        except ValueError:
            continue
        # German grades range 1.0–5.0; ignore out-of-range values.
...
class StudentService:
    ...
    async def upload_transcript(
        ...
        _logger.info(
            "Step 1/5 — PDF text extracted: %d characters (user_id=%d)",
            len(raw_text), user_id,
        )

        # 2. Ask the LLM to parse the transcript.
        _logger.info(
            "Step 2/5 — Sending transcript to LLM for structured extraction "
            "(model=%s user_id=%d)",
            self._settings.effective_extract_model, user_id,
        )
        parse_result = await self._parse_transcript_with_llm(raw_text)
        _logger.info(
            "Step 2/5 — LLM extraction complete: %d courses parsed (user_id=%d)",
            len(parse_result.courses), user_id,
        )

        # 3. Recompute GPA server-side (don't blindly trust the LLM's arithmetic).
        gpa = _compute_gpa(parse_result.courses)
        _logger.info("Step 3/5 — GPA computed: %s (user_id=%d)", gpa, user_id)

        # 4. Embed course profile for chair ANN search.
        _logger.info(
        ...
        if profile_embedding is not None:
            _logger.info(
                "Step 4/5 — Embedding created: dim=%d (user_id=%d)",
                len(profile_embedding), user_id,
            )
        else:
        ...
        # Eagerly load courses for the response.
        student = await self._student_repo.get_by_user_id(user_id)
    ...
    async def _parse_transcript_with_llm(self, raw_text: str) -> TranscriptParseResult:
        ...
        try:
            ...
            _logger.error("Transcript parse result failed validation: %s", data)
            raise BadRequestException(
                f"Transcript parse result had unexpected structure: {exc}"
            ) from exc
    ...
    async def _embed_course_profile(
        ...
        if not courses:
            ...
            return None
        # Concatenate course names (with credits) as the text to embed.
        parts = [
        ...
        text = "; ".join(parts)
        _logger.info(
            "Embedding course profile: %d courses, %d chars of text",
            len(courses), len(text),
        )
        try:
            ...
            _logger.warning(
                "Failed to embed course profile; profile_embedding will be null. Reason: %s", exc
            )
            return None


async def _extract_pdf_text(pdf_bytes: bytes) -> str:
    """Extract plain text from a PDF using pypdf (sync, run in thread pool)."""
    import asyncio
    import io
    ...
    def _sync_extract(data: bytes) -> str:
        try:
            ...
            raise RuntimeError(
                "pypdf is not installed. Add it to pyproject.toml: pypdf>=4.0"
            ) from exc
        reader = PdfReader(io.BytesIO(data))
        _logger.info("PDF has %d page(s)", len(reader.pages))
        pages = [page.extract_text() or "" for page in reader.pages]
        total_chars = sum(len(p) for p in pages)
        _logger.info("PDF text extracted: %d pages, %d total characters", len(pages), total_chars)
        return "\n".join(pages)
